...
import {
  ...
  Search, Play, Gift, ArrowRight, ShoppingBag, Sun, Moon, Bell, RotateCcw,
  Plus, Bookmark, Star, User, Crown, Volume2, VolumeX, Sparkles,
  MessageSquare, EyeOff, RefreshCw, Send, FileText, ShieldCheck,
  Smile, Link2, UserPlus, ShieldAlert, Loader2, MapPin, LocateFixed,
} from "lucide-react";
...
// Offline fallback, used only if the live Bible API cannot be reached and nothing is cached yet
const BIBLE_SAMPLE = {
  "Salmos-23": [
    { v: 1, t: "O Senhor é o meu pastor; nada me faltará." },
    { v: 2, t: "Em verdes pastagens me faz repousar e para águas tranquilas me conduz." },
    { v: 3, t: "Restaura-me a alma e me guia nas veredas da justiça por amor do seu nome." },
    { v: 4, t: "Ainda que eu ande pelo vale da sombra da morte, não temerei mal algum, pois tu estás comigo." },
    { v: 5, t: "Preparas um banquete para mim à vista dos meus adversários. Unges a minha cabeça com óleo; o meu cálice transborda." },
    { v: 6, t: "Sei que a bondade e o amor hão de me acompanhar todos os dias da minha vida, e voltarei à casa do Senhor enquanto eu viver." },
  ],
  "Gênesis-1": [
    { v: 1, t: "No princípio, Deus criou os céus e a terra." },
    { v: 2, t: "A terra era sem forma e vazia, e havia trevas sobre a face do abismo; e o Espírito de Deus se movia sobre a face das águas." },
  ...
  "João-3": [
    ...
    { v: 17, t: "Pois Deus enviou o seu Filho ao mundo não para condenar o mundo, mas para que o mundo fosse salvo por meio dele." },
  ],
};
...
// Real call to the Anthropic API (no key needed in this environment) — reads the person's journal entry
// and asks Claude to suggest one relevant Bible passage, returned as strict JSON.
...
// Real automatic moderation call — every post goes through this before it is saved to the shared board.
...
Texto: """${text}"""`;
  ...
  if (!response.ok) throw new Error("moderation API status " + response.status);
  const data = await response.json();
  const raw = (data.content || []).map((b) => b.text || "").join("").replace(/```json|```/g, "").trim();
...
const IMAGE_URL_RE = /\.(jpg|jpeg|png|gif|webp)(\?[^\s]*)?$/i;
function extractLink(text) { const m = text.match(URL_RE); return m ? m[0] : null; }
function isImageLink(url) { return !!url && IMAGE_URL_RE.test(url.split("?")[0]); }
...
function playChime(soundOn, variant = "confirm") {
  ...
  } catch (e) { /* Web Audio unavailable — stay silent, never block the interaction */ }
}
    ...
    <Modal title={user ? "Seu perfil" : "Entrar"} onClose={onClose}>
      {user ? (
        <div className="text-center py-2">
          ...
          <p className="text-xs mt-1" style={{ color: T.textMuted }}>Conectado nesta demonstração</p>
        ...
        <div>
          <button onClick={() => onSignIn(name.trim() || "Convidado")} className="w-full flex items-center justify-center gap-2.5 rounded-full py-3 text-sm font-medium mb-3 transition active:scale-95" style={{ backgroundColor: "#fff", border: "1px solid #dadce0", color: "#3c4043" }}>
          ...
          <p className="text-xs mb-2" style={{ color: T.textMuted }}>ou digite um nome para esta demonstração:</p>
          ...
          <p className="text-xs mt-4 leading-relaxed" style={{ color: T.textMuted }}>
            🔧 Demonstração: login real do Google exige um Client ID OAuth próprio (Google Cloud Console) e o script oficial deles, que não roda dentro deste preview em iframe. Com Firebase Authentication, ou o plugin nativo de Google Sign-In do Capacitor já como app instalado, esse mesmo botão passa a autenticar de verdade.
          </p>
        </div>
      )}
    </Modal>
  ...
  return (
    <div className="px-5 py-3 space-y-4">
      <div className="flex items-center gap-2.5">
        <button onClick={onOpenProfile} className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          {user ? <span className="text-sm font-semibold" style={{ color: T.accent }}>{user.name.charAt(0).toUpperCase()}</span> : <User size={16} style={{ color: T.accent }} />}
        </button>
...
function BibleScreen({ book, setBook, chapter, setChapter, cache, setCache, version, setVersion, favoriteVerses, toggleFavoriteVerse, lastRead, onMarkLastRead, isPremium }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch (e) { /* not cached yet — will fetch live below */ }
      try {
      ...
      {showFavorites ? (
        ...
        <>
          ...
          <div className="rounded-2xl p-4" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
            ...
            {status === "loading" && !verses && <VerseSkeleton />}
              ...
              <div>
                {fallback ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs pt-1" style={{ color: T.textMuted }}>A API não respondeu agora — mostrando texto de amostra salvo no app.</p>
                  </div>
                ) : (
                  <div className="text-center py-3">
                    <p className="text-xs mb-2 leading-relaxed" style={{ color: T.textMuted }}>Não foi possível carregar este capítulo agora (a API pública gratuita limita a 20 pedidos por hora).</p>
...
function Avatar({ name, photoUrl, size = 28 }) {
  ...
  if (photoUrl && !broken) {
    return <img src={photoUrl} onError={() => setBroken(true)} alt={name} className="rounded-full object-cover shrink-0" style={{ width: size, height: size }} />;
  }
  return (
...
function EmojiPicker({ onSelect, onClose }) {
  const T = useTheme();
  const [group, setGroup] = useState("Comum");
  return (
    <div className="rounded-2xl p-3 mt-2" style={{ backgroundColor: T.cardAlt, border: `1px solid ${T.border}` }}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex gap-1.5">
          {Object.keys(EMOJI_GROUPS).map((g) => (<button key={g} onClick={() => setGroup(g)} className="text-xs px-2.5 py-1 rounded-full" style={{ backgroundColor: group === g ? T.accent : T.card, color: group === g ? T.onAccent : T.textMuted }}>{g}</button>))}
        </div>
        <button onClick={onClose}><X size={14} style={{ color: T.textMuted }} /></button>
      </div>
      <div className="grid grid-cols-8 gap-1">
        {EMOJI_GROUPS[group].map((e, i) => (<button key={i} onClick={() => onSelect(e)} className="text-lg py-1 rounded-lg transition active:scale-90">{e}</button>))}
      </div>
    </div>
  );
}
...
function PostText({ text }) {
  ...
  const beforeLink = link ? text.split(link)[0] : text;
  const afterLink = link ? text.split(link).slice(1).join(link) : "";
  return (
    <div>
      <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: T.text }}>
        {beforeLink}
        {link && (<a href={link} target="_blank" rel="noopener noreferrer" className="underline" style={{ color: T.accent }}>{link}</a>)}
        {afterLink}
      </p>
      {link && isImageLink(link) && (
        <img src={link} alt="" className="mt-2 rounded-xl w-full object-cover" style={{ maxHeight: 220 }} onError={(e) => { e.target.style.display = "none"; }} />
      )}
    </div>
  );
}
...
function DMThread({ myName, otherName, onBack }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      try { const res = await window.storage.get(key, true); current = res && res.value ? JSON.parse(res.value) : []; } catch (e) { current = []; }
  ...
  return (
    <div className="flex flex-col" style={{ height: "100%" }}>
      ...
      <div className="rounded-xl p-2.5 mb-2 flex items-start gap-2" style={{ backgroundColor: T.cardAlt }}>
        ...
        <p className="text-xs leading-relaxed" style={{ color: T.textMuted }}>Esta conversa <b>não é criptografada nem privada de verdade</b> — não é um sistema de contas real. Evite compartilhar dados sensíveis aqui.</p>
      </div>
      ...
      <div className="pt-2">
        ...
        <div className="flex items-center gap-2 mt-2">
          <button onClick={() => setShowEmoji((s) => !s)}><Smile size={19} style={{ color: T.accent }} /></button>
...
function CommunityScreen({ user, hiddenPosts, onHidePost, likedPosts, onToggleLike, contacts, addContact }) {
  const T = useTheme();
  ...
  const load = async () => {
    ...
    try {
      ...
      setPosts([]); // nobody has posted yet — empty board, not a real error
    } finally {
      setStatus("idle");
    }
  };
  ...
  const submitPost = async () => {
    ...
    try {
      ...
      setPosts(updated);
      setText("");
    } catch (e) { /* keep their draft text so nothing is lost */ }
  ...
  const like = async (id) => {
    ...
    try {
      ...
      await window.storage.set(COMMUNITY_KEY, JSON.stringify(updated), true);
    } catch (e) { /* local optimistic state already applied */ }
  };
  ...
  return (
    <div className="px-5 py-3 space-y-4">
      <div className="rounded-xl p-3 flex items-start gap-2" style={{ backgroundColor: T.cardAlt }}>
        ...
        <p className="text-xs leading-relaxed" style={{ color: T.textMuted }}>Mural público — visível para todas as pessoas que usam este app, e não é moderado em tempo real. Seja gentil.</p>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch (e) { /* first time opening — nothing saved yet */ }
  ...
  const signIn = (name) => setUser({ name }); const signOut = () => setUser(null);
